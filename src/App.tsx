...
return (
        <div className="flex flex-col h-full bg-gray-50">
             <div className="flex bg-white shadow-sm pt-[env(safe-area-inset-top)] z-10 relative">
                {/* [修改] 添加 font-cute 类名 */}
    <button onClick={() => setActiveTab('solo')} className={`flex-1 py-2 font-bold text-sm font-cute transition-colors ${activeTab === 'solo' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-400'}`}>独自记录</button>
    <button onClick={() => setActiveTab('joint')} className={`flex-1 py-2 font-bold text-sm font-cute transition-colors ${activeTab === 'joint' ? 'text-rose-500 border-b-2 border-rose-500' : 'text-gray-400'}`}>双方裁决</button>
            </div>
